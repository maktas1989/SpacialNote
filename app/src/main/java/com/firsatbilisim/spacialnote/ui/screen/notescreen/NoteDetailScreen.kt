...
@Composable
fun NoteDetailScreen(noteId: Int, navController: NavController, viewModel: NoteViewModel = viewModel()) {

    var noteTitle by remember { mutableStateOf("") }
    var noteDescription by remember { mutableStateOf("") }
    var noteUpdateAt = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(Date())

    val localFocusManager = LocalFocusManager.current
    val context = LocalContext.current // Context almak için

    val noteList by viewModel.selectedNote.collectAsState()
                ...
                Button(
                    ...
                    onClick = {
                        if (noteTitle.isEmpty()) {
                            ...
                            noteList?.let {
                                val updatedNote = it.copy(title = noteTitle, description = noteDescription, updatedAt = noteUpdateAt)
